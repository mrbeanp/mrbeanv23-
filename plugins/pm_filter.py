...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
        ...
        buttons = [
                    [
                        InlineKeyboardButton('🎖 DEPLOY YOURS 🎖', url='https://t.me/mallu_movie_search')
                    ]
                    ]
...
async def auto_filter(client, msg, spoll=False):
    ...
    if settings["button"]:
        btn = [
            [
                InlineKeyboardButton(
                    text=f"𒊹︎︎︎{get_size(file.file_size)}➪ {file.file_name}", callback_data=f'{pre}#{file.file_id}'
                ),
            ]
            for file in files
        ]
    ...
    if offset != "":
        ...
        btn.append(
            [InlineKeyboardButton(text=f"❁ 𝗣𝗮𝗴𝗲 1/{round(int(total_results) / 7)}", callback_data="pages"),
